...
import pandas as pd
from pandas.tseries.offsets import CustomBusinessMonthBegin, CustomBusinessDay
...
IDX = mcal.get_calendar('XIDX')
CBD = IDX.holidays()
IDX_business_day = CustomBusinessDay(holidays=CBD.holidays)
...
class CustomIDHolidayCalendar(AbstractHolidayCalendar):
    rules = [
        Holiday("New Year's Day", month=1, day=1),
        Holiday("Labor Day", month=5, day=1),
        Holiday("Christmas Day", month=12, day=25),
        Holiday("Eid al-Fitr 1", month=6, day=15),
        Holiday("Eid al-Fitr 2", month=6, day=16),
        Holiday("Eid al-Adha", month=7, day=20),
        Holiday("Nyepi", month=3, day=22),
        Holiday("Good Friday", month=4, day=10),
        Holiday("Maulid Nabi", month=10, day=29),
        Holiday("Vesak", month=5, day=15),
        Holiday("Ascension Day", month=5, day=29),
        Holiday("Chinese New Year", month=2, day=10),
    ]
...
def get_yf_time_series(yf, symbol, freq=IDX_business_day, fut=False):
    """
    Download historical time series data from Yahoo Finance for a given symbol and save to CSV.
    
    Parameters:
    - yf: yfinance module
    - symbol: Ticker symbol (e.g., 'BBCA.JK' for IDX stocks or '^JKSE' for index)
    - freq: Frequency offset, typically CustomBusinessDay with XIDX holidays
    - fut: Boolean indicating if the data is for futures (uses YF_F_COLUMNS if True)
    ...
    try:
        ...
        if df.empty:
            print(f"No data downloaded for {symbol} from {start_date.date()} to {end_date.date()}")
            return pd.DataFrame()
    ...
    if fut:
        if len(df.columns) == len(YF_F_COLUMNS):
            df.columns = YF_F_COLUMNS
        else:
            print(" Column mismatch for futures data, returning original columns.")
    else:
        missing_cols = [col for col in expected_cols if col not in df.columns]
        if missing_cols:
            print(f" Missing columns: {missing_cols}. Returning original DataFrame.")
            return df
        df = df[expected_cols]
    ...
    output_file = output_dir / f'{symbol}_XIDX_{start_file}_{end_file}.csv'
    df.to_csv(output_file)
    print(f"Saved data to {output_file}")

    return df

def load_yf_time_series(yf, symbol, freq=IDX_business_day, impute=True):
    """
    Load historical time series data from a CSV file for a given symbol.
    
    Parameters:
    - yf: yfinance module (unused but kept for consistency)
    - symbol: Ticker symbol (e.g., 'BBCA.JK' for IDX stocks)
    - freq: Frequency offset, typically CustomBusinessDay with XIDX holidays
    - impute: Boolean indicating whether to interpolate missing data
    ...
    csv_file = TOP / 'data' / f'{symbol}_XIDX_{start_file}_{end_file}.csv'
    ...
    try:
        ...
        if 'Date' in df.columns:
            ...
            print(f"'date' column not found in CSV: {csv_file}")
            return pd.DataFrame()
